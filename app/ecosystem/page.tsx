...
export default function Page() {
  ...
  return (
    <main className="scroll-smooth min-h-screen">
      ...
      <section className="pt-8 sm:pt-12 pb-12 sm:pb-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-gray-600 text-sm sm:text-base mt-2 sm:mt-4">Ownership distribution of German UDS</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            {shares.map((share, index) => (
              <motion.div
                ...
                className="relative rounded-2xl border border-gray-100 bg-gray-50/50 p-4 sm:p-6 hover:bg-white hover:shadow-lg hover:border-gray-200 transition-all duration-300 overflow-hidden"
              >
                <div className={`absolute top-0 left-0 w-1 h-full ${share.color}`} />
                <div className="pl-3 sm:pl-4">
                  <p className={`text-3xl sm:text-4xl font-bold mb-1 ${index === 0 ? 'text-[#0066FF]' : index === 1 ? 'text-orange-600' : 'text-slate-600'}`}>
                    {share.percentage}%
                  </p>
                  <h3 className="font-semibold text-gray-900 text-sm sm:text-base mb-2 sm:mb-3">{share.holder}</h3>
                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">{share.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
